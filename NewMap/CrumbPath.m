...
- (MKMapRect)addCoordinate:(CLLocationCoordinate2D)newCoord boundingMapRectChanged:(BOOL *)boundingMapRectChangedOut
{
    ...
    self.userDistance += metersApart;
    if (self.userDistance % 500 == 0 && self.userDistance != 0) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[VoiceUtil shared] speekSomething:[NSString stringWithFormat:@"you had run %d meters, star love pig", (int)self.userDistance]];
            [[NSNotificationCenter defaultCenter] postNotificationName:BEGIN_FLASH object:nil];
        });
